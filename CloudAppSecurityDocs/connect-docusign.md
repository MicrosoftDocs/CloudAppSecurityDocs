...
- Due to DocuSign’s API limitation, in order to have SaaS Security Posture management (SSPM) support there is a need to re-connect it with additional permissions: **"account_read account_write and user_read organization_read"** .
...
1. Note that SaaS Security Posture Management (SSPM) data will be shown in the Microsoft Defender portal under "Secue Score" page, [see here additional information](/defender-cloud-apps/security-saas).
...
1. For SaaS Security Posture Management (SSPM) support, provided credentials must have these permissions - **"account_read account_write and user_read organization_read"**.
