...
5. Deploy the collector image on the hosting machine by importing the collector configuration. Import the configuration by copying the run command generated in the portal. If you need to configure a proxy, add the proxy IP address and port number to the command generated by the portal in the argument `-e "PROXY="`. For example, if your proxy details are 192.168.10.1:8080, your updated run command might look like this:
