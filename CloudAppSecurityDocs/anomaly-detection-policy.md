...
This detection takes advantage of the Defender for Cloud Apps ability to monitor user behavior across apps, allowing identification of the regular activity of the user, the fact that the account was terminated, and actual activity on other apps. For example, an employee whose Azure AD account was terminated, but still has access to the corporate AWS infrastructure, has the potential to cause large-scale damage.
...
> [!NOTE]
> A user is considered terminated when their account is deleted in Azure Active Directory. Existing user accounts can't be marked as terminated.
