---
title: Classic portal -  Deploy Conditional Access App Control for any web app using PingOne
description: Classic portal -  This article provides information about how to deploy the Microsoft Defender for Cloud Apps Conditional Access App Control for any web app using the PingOne identity provider.
ms.date: 01/19/2023
ms.topic: how-to
ROBOTS: NOINDEX
---
# Classic portal: Deploy Conditional Access App Control for any web app using PingOne as the identity provider (IdP)
...
1. Under **Single Sign-On Settings**, select the name of your existing SAML 2.0 configuration.
    ...
    ![Select Salesforce SSO settings.](media/proxy-idp-pingone/classic-idp-pingone-sf-select-sso-settings.png)
...
1. On the **SAML Single Sign-On Setting** page, make a note of the Salesforce **Login URL**. You'll need this later.
    ...
    ![Select Salesforce SSO login URL.](media/proxy-idp-pingone/classic-idp-pingone-sf-copy-saml-sso-login-url.png)
...
1. On the **APP INFORMATION** page, select **Fill in data manually**, in the **Assertion consumer service URL** enter the Salesforce **Login URL** you noted earlier, and then select **Next**.
    ...
    ![Manually fill in Salesforce SAML information.](media/proxy-idp-pingone/classic-idp-pingone-cas-sf-app-info.png)
...
1. On the **SSO Attribute Mapping** page, make a note of the SAML_SUBJECT attribute and value, and then download the **Signing Certificate** and **SAML Metadata** files.
    ...
    ![Note existing Salesforce app's attributes.](media/proxy-idp-pingone/classic-idp-pingone-sf-app-copy-saml-sso-attributes.png)

1. Open the SAML metadata file and make a note of the PingOne **SingleSignOnService Location**. You'll need this later.

    ![Note existing Salesforce app's SSO service location.](media/proxy-idp-pingone/classic-idp-pingone-sf-app-copy-saml-sso-service-location.png)

1. On the **Group Access** page, make a note of the assigned groups.

    ![Note existing Salesforce app's assigned groups.](media/proxy-idp-pingone/classic-idp-pingone-sf-app-copy-saml-sso-user-groups.png)
...
![Add SAML app with your identity provider.](media/classic-proxy-deploy-add-idp-get-conf.png)
...
1. Create a **New SAML Application**.
    ...
    ![In PingOne, create new custom Salesforce app.](media/proxy-idp-pingone/classic-idp-pingone-sf-custom-app-new.png)
...
1. On the **Application Details** page, fill out the form, and then select **Continue to Next Step**.
    ...
    ![Fill out the custom app details.](media/proxy-idp-pingone/classic-idp-pingone-sf-custom-app-details.png)
...
1. On the **Application Configuration** page, do the following, and then select **Continue to Next Step**.
    ...
    ![Configure custom app with Salesforce SAML details.](media/proxy-idp-pingone/classic-idp-pingone-sf-custom-app-set-saml-sso-properties.png)
...
1. On the **SSO Attribute Mapping** page, add the existing Salesforce app's **SAML_SUBJECT** attribute and value you noted earlier, and then select **Continue to Next Step**.
    ...
    ![Add attributes to custom Salesforce app.](media/proxy-idp-pingone/classic-idp-pingone-sf-custom-app-set-saml-sso-attributes.png)

1. On the **Group Access** page, add the existing Salesforce app's groups you noted earlier, and complete the configuration.

    ![Assign groups to custom Salesforce app.](media/proxy-idp-pingone/classic-idp-pingone-sf-custom-app-set-saml-sso-user-groups.png)
...
1. On the next page, select **Fill in data manually**, do the following, and then select **Next**.
    ...
    ![Add SSO service URL and SAML certificate.](media/proxy-idp-pingone/classic-idp-pingone-cas-sf-app-idp-info.png)
...
1. On the next page, make a note of the following information, and then select **Next**. You'll need the information later.
    ...
    ![In Defender for Cloud Apps, note SSO URL and attributes.](media/proxy-idp-pingone/classic-idp-pingone-cas-get-sf-app-external-config.png)
...
1. In PingOne, locate and edit the custom Salesforce app.
    ...
    ![Locate and edit custom Salesforce app.](media/proxy-idp-pingone/classic-idp-pingone-sf-custom-app-edit.png)
...
1. In the **Assertion Consumer Service (ACS)** field, replace the URL with the Defender for Cloud Apps single sign-on URL you noted earlier, and then select **Next**.
    ...
    ![Replace ACS in custom Salesforce app.](media/proxy-idp-pingone/classic-idp-pingone-sf-custom-app-replace-saml-sso-properties.png)

1. Add the Defender for Cloud Apps attributes and values you noted earlier to the app's properties.

    ![Add Defender for Cloud Apps attributes to custom Salesforce app.](media/proxy-idp-pingone/classic-idp-pingone-sf-custom-app-replace-saml-sso-attributes.png)
...
![Note the Defender for Cloud Apps SAML SSO URL and download the certificate.](media/proxy-idp-pingone/classic-idp-pingone-cas-sf-app-changes.png)
...
1. Select **Save**.
    ...
    ![Update custom Salesforce app with Defender for Cloud Apps SAML details.](media/proxy-idp-pingone/classic-idp-pingone-sf-custom-app-changes.png)
